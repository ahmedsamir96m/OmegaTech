...
const AddressForm = ({ checkoutToken, nextCheckout }) => {
  ...
  const options = shippingOptions.map((shippingOp) => ({
    id: shippingOp.id,
    label: `${shippingOp.description} - (${shippingOp.price.formatted_with_symbol})`
  }))
  ...
  return (
    <div>
      ...
      <FormProvider {...methods}>
        <form onSubmit={methods.handleSubmit((shippingData) => {nextCheckout({...shippingData, shippingCountry, shippingSubdivision, shippingOption})})}>
